 ...
 * which is covered by an Apache 2 license https://github.com/whylabs/langkit/blob/main/LICENSE
 */
class LangkitPromptInjectionQueryTransformer implements QueryTransformer
{
    // These prompts come from https://github.com/whylabs/langkit/blob/main/langkit/themes.json
    final public const JAILBREAK_PROMPTS = [
     ...
     * @var Document[]
     */
    private array $jailbreakPromptDocuments = [];

    public function __construct(
        private readonly EmbeddingGeneratorInterface $embeddingGenerator,
        private readonly Distance $distance = new CosineDistance(),
        private readonly float $nearestDistance = 0.26,
    ) {
    }
    ...
    {
        $this->computeBaseDistance();

        $embeddedQuery = $this->embeddingGenerator->embedText($query);

        foreach ($this->jailbreakPromptDocuments as $document) {
            $currentDistance = $this->getMeasure($document, $embeddedQuery);
            $msg = " - Near Distance: $this->nearestDistance, Current Distance: $currentDistance, Compared malicious prompt: $document->content";
            if ($currentDistance < $this->nearestDistance) {
                throw new SecurityException('Prompt flagged as insecure: '.$query.$msg);
            }
        }
    ...
    private function computeBaseDistance(): void
    {
        if ($this->jailbreakPromptDocuments === []) {
            $this->jailbreakPromptDocuments = DocumentUtils::documents(...self::JAILBREAK_PROMPTS);
            foreach ($this->jailbreakPromptDocuments as $document) {
                $document->embedding = $this->embeddingGenerator->embedText($document->content);
            }
        }
    }

    /**
     * @param  float[]  $embeddedQuery
     */
    public function getMeasure(Document $document, array $embeddedQuery): float
    {
        if ($document->embedding === null) {
            throw new \Exception('Unexpected null embedding: '.print_r($document, true));
        }

        return $this->distance->measure($document->embedding, $embeddedQuery);
    }
}
